    ...
    function index()
    {
		...
		if($user_info != ""){
			if($user_info[integral] < $goods_info[int_amount]){
				$this->show_warning('您当前的积分为：<strong>'.$user_info[integral].'分</strong><br>不足以购买这些商品，请重新选购');
				return;
			}
		}
		...
		//免费会员无法购买纯积分商品,无法购买官方店铺以外店铺的产品！
		$llg_id = array(275);   //零乐购店铺ID
		
		if($user_info['grade'] == '免费会员'){
			foreach($goods_info['items'] as $value){
				if($value['subtotal'] == 0){
					$this->show_warning('下单失败，订单中有纯积分商品，免费会员无法购买纯积分商品！');
					return;
				}
			}
			
			if(!in_array($goods_info['store_id'],$llg_id)){
				$this->show_warning('下单失败，免费会员只能购买官方店铺的商品！');
				return;
			}
		}
        ...
        else
        {
			...
			$data = array(
				...
				'integral' => $order_info['int_amount'],
				 ...
				 'content' => '下订单'.$order_info['order_sn']
			);
			$recode_mod->add($data);
