...
class Seller_orderApp extends StoreadminbaseApp
{
    ...
    function cancel_order()
    {
		...
		foreach($order_info as $value){
			if( date('Y-m', $value['add_time']) != date('Y-m', gmtime()) ){      //如果不是当月的则进入判断
				$the_user = $user_mod->get_info($value['buyer_id']);
				if($the_user['grade'] == '免费会员'){    						 //如果下订单的用户是免费会员那么报错
					$this->show_warning("对不起，取消失败！只能取消免费会员的当月订单！");
					return;
				}
			}
