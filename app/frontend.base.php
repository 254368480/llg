...
class FrontendApp extends ECBaseApp
{
    ...
    function login()
    {
        ...
        else
        {
            ...
            {
				$mod_user =& m('member');
				$user_info = $mod_user->get_info($user_id);
				if($user_info['grade'] != "免费会员"){
					if($user_info['login'] == 1){
						$c_time = gmtime();
						if($user_info['use_time'] <= $c_time){
							$date = array('login' => 0);
							$mod_user->edit($user_id, $date);
							$this->show_warning("登录失败，您的账号已到期，现进入冻结状态，请续费！");
							return;
						}
					}
					else{
...
class MemberbaseApp extends MallbaseApp
{
    ...
    {
        /* 只有登录的用户才可访问 */
        if (!$this->visitor->has_login && !in_array(ACT, array('login', 'register', 'check_user', 'check_deposit')))
        {
            if (!IS_AJAX)
            {
                header('Location:index.php?app=member&act=login&ret_url=' . rawurlencode($_SERVER['PHP_SELF'] . '?' . $_SERVER['QUERY_STRING']));
    ...
    function _get_member_menu()
    {
        ...
        $menu['my_ecmall'] = array(
            ...
            'submenu'   => array(
                ...
                'my_profile'  => array(
				 ...
				 'my_deposit'  => array(
                    'text'  => 我的业务,
                    'url'   => 'index.php?app=member&act=deposit',
                    'name'  => 'my_deposit',
                    'icon'  => 'ico4',
		...
		$menu['my_integral'] = array(
            'name'  => 'my_integral',
            'text'  => '积分管理',
            'submenu'   => array(
                'integral_to'  => array(
                       'text'  => "我要转分",
                       'url'   => 'index.php?app=member&act=integral_to',
                       'name'  => 'integral_to',
                       'icon'  => 'ico4',
                ),
				'integral_recode'  => array(
                       'text'  => "积分记录",
                       'url'   => 'index.php?app=member&act=integral_recode',
                       'name'  => 'integral_recode',
                       'icon'  => 'ico4',
                ),
		...
		$menu['my_money'] = array(
            ...
            'submenu'   => array(
                'money_to'  => array(
                       'text'  => "我要转账",
                       'url'   => 'index.php?app=member&act=money_to',
                       'name'  => 'money_to',
                       'icon'  => 'ico4',
                ),
				'money_recode'  => array(
                       'text'  => "转账记录",
                       'url'   => 'index.php?app=member&act=money_recode',
                       'name'  => 'money_recode',
                       'icon'  => 'ico4',
                ),
