...
class CartApp extends MallbaseApp
{
    ...
    function add()
    {
        ...
        $spec_info  =  $spec_model->get(array(
            'fields'        => 'g.store_id, g.goods_id, g.goods_name, g.integral, g.spec_name_1, g.spec_name_2, g.default_image, gs.spec_1, gs.spec_2, gs.stock, gs.price',
            'conditions'    => $spec_id,
    ...
    function drop()
    {
        ...
        $this->json_result(array(
			...
			'int_amount'=>  $cart_status['carts'][$store_id]['int_amount'], 
    ...
    function update()
    {
		...
		$inttotal   = 	$quantity * $cart_spec_info['integral'];
    ...
    function _get_cart_status()
    {
        ...
        foreach ($carts as $store_id => $cart)
        {
			...
			$data['status']['int_amount'] += $cart['int_amount'];
    ...
    function _get_carts($store_id = 0)
    {
        ...
        foreach ($cart_items as $item)
        {
			...
			$item['inttotal']   = $item['integral'] * $item['quantity'];
			...
			$carts[$item['store_id']]['int_amount'] += $item['inttotal'];   //各店铺的总积分
