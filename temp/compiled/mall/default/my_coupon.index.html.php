...
?>
                <tr class="line<?php if (($this->_foreach['v']['iteration'] == $this->_foreach['v']['total'])): ?> last_line<?php endif; ?>">
                    ...
                    <td class="align4"><?php if ($this->_var['coupon']['coupon_value']): ?><?php echo $this->_var['coupon']['coupon_value']; ?><?php else: ?>没有限制<?php endif; ?></td>
                    <td><?php echo local_date("Y-m-d",$this->_var['coupon']['start_time']); ?> 至 <?php if ($this->_var['coupon']['end_time']): ?><?php echo local_date("Y-m-d",$this->_var['coupon']['end_time']); ?><?php else: ?>没有限制<?php endif; ?></td>
                    <td><?php if ($this->_var['coupon']['min_amount']): ?><?php echo sprintf('一次购物满 %s ', $this->_var['coupon']['min_amount']); ?><?php else: ?>没有限制<?php endif; ?></td>
                    <td><a href="<?php echo url('app=store&id=' . $this->_var['coupon']['store_id']. ''); ?>"><?php echo $this->_var['coupon']['store_name']; ?></a></td>
                    <td class="align4"><?php if ($this->_var['coupon']['remain_times'] == - 1): ?>没有限制<?php else: ?><?php echo $this->_var['coupon']['remain_times']; ?><?php endif; ?></td>
