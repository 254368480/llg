...
class DefaultPassportUser extends BasePassportUser
{
    ...
    /* 注册 */
    function register($user_name, $password, $email, $msn, $deposit, $sfz, $local_data = array())
    {
		...
		//根据推荐人等级，决定注册人的分销商，代理商，间接推荐人
		$user_mod =& m('member');
		$sql = "select * FROM {$user_mod->table} WHERE user_name = '$deposit'";
		$deposit_user = $user_mod->getRow($sql);
		if($deposit_user != ""){
			if($deposit_user['grade'] == "代理商"){
					$dls = $deposit_user['user_name'];
					$fxs = '';
			}
			if($deposit_user['grade'] == '分销商' ){
					$dls = $deposit_user['dls'];
					$fxs = $deposit_user['user_name'];
					$jjtjr = $deposit_user['jjtjr'].",".$deposit_user['deposit'];
			}
			if( ($deposit_user['grade'] == "VIP会员") || ($deposit_user['grade'] == "免费会员") ){
					$fxs = $deposit_user['fxs'];
					$dls = $deposit_user['dls'];
					$jjtjr = $deposit_user['jjtjr'].",".$deposit_user['deposit'];
			}
		}
		else{
			$deposit_user['user_name'] = "公司";
		}
		$end_time = gmtime();
		$Y=date('Y',$end_time);
		$M=date('m',$end_time);
		$D=date('d',$end_time);
		$end_time = mktime(0,0,0,$M,$D,($Y+1));
        $local_data['user_name']    = $user_name;
        $local_data['password']     = md5($password);
        $local_data['email']        = $email;
		$local_data['im_msn']       = $msn;
		$local_data['deposit']		= $deposit_user['user_name'];
		$local_data['sfz']          = $sfz;
		$local_data['fxs']          = $fxs;
		$local_data['dls']          = $dls;
		$local_data['jjtjr']        = $jjtjr;
		$local_data['use_time']     = $end_time;
